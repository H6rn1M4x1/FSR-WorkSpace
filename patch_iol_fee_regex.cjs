...
const regex = /\/\/ InvertirOnline \(IOL\) Tariffs calculation based on selected ticker\/panel[\s\S]*?const iolFeeInfo = useMemo\(\(\) => \{[\s\S]*?\}, \[formValorUnitarioPesos, formCantidad, formDolarCCL, formSplit, formTicker, allCotizaciones\]\);/;

const newBlock = `  // InvertirOnline (IOL) Tariffs calculation based on selected ticker/panel
