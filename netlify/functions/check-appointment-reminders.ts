...
const NIGHT_BEFORE_SUMMARY_HOUR = 22; // 10 PM the day before — preview of tomorrow
...
function getArgentinaDateStr(d: Date): string {
  const fmt = new Intl.DateTimeFormat("en-CA", {
    timeZone: "America/Argentina/Buenos_Aires",
    year: "numeric",
    month: "2-digit",
...
async function buildSummarySections(
  targetDateStr: string,
  now: Date,
  snaps: { turnosSnap: any; paymentsSnap: any; examenesSnap: any; tasksSnap: any; medsSnap: any; dispSnap: any; orgSnap: any; platosSnap: any }
): Promise<{ title: string; wa: string[]; push: string[] }[]> {
  const { turnosSnap, paymentsSnap, examenesSnap, tasksSnap, medsSnap, dispSnap, orgSnap, platosSnap } = snaps;
  const sections: { title: string; wa: string[]; push: string[] }[] = [];

  // --- Turnos ---
  const turnosDia = turnosSnap.docs
    .map((d: any) => d.data() as any)
    .filter((tc: any) => !tc.estatus && String(tc.fecha || "").startsWith(targetDateStr))
    .sort((a: any, b: any) => String(a.fecha).localeCompare(String(b.fecha)));
  if (turnosDia.length > 0) {
    sections.push({
      title: "📅 Turnos",
      wa: turnosDia.map((tc: any) => {
        const t = String(tc.fecha).split("T")[1];
        return `• ${t ? `*${t.slice(0, 5)}hs* - ` : ""}${tc.descripcion}${tc.lugar ? ` _(${tc.lugar})_` : ""}`;
      }),
      push: turnosDia.map((tc: any) => {
        const t = String(tc.fecha).split("T")[1];
  ...
  // --- Finanzas: pagos que vencen ese día ---
  const pagosDia = paymentsSnap.docs
    .map((d: any) => d.data() as any)
    .filter((p: any) => !p.pago && String(p.fechaVencimiento || "").startsWith(targetDateStr));
  if (pagosDia.length > 0) {
    sections.push({
      title: "💵 Finanzas",
      wa: pagosDia.map((p: any) => `• *${p.descripcion}* - $${(p.montoAPagar || 0).toLocaleString("es-AR")} _(${p.categoria || "General"})_`),
      push: pagosDia.map((p: any) => `• ${p.descripcion} - $${(p.montoAPagar || 0).toLocaleString("es-AR")} (${p.categoria || "General"})`),
    });
  }

  // --- Universidad: exámenes ese día + trabajos que vencen ese día ---
  const examenesDia = examenesSnap.docs.map((d: any) => d.data() as any).filter((e: any) => String(e.fecha || "").startsWith(targetDateStr));
  const trabajosDia = tasksSnap.docs.map((d: any) => d.data() as any).filter((t: any) => !t.completed && String(t.dueDate || "").startsWith(targetDateStr));
  if (examenesDia.length > 0 || trabajosDia.length > 0) {
    sections.push({
      title: "🎓 Universidad",
      wa: [
        ...examenesDia.map((e: any) => `• *Examen: ${e.materia}*${e.hora ? ` a las ${e.hora}hs` : ""}${e.aula ? ` _(${e.aula})_` : ""}`),
        ...trabajosDia.map((t: any) => `• *Trabajo: ${t.title}*`),
      ],
      push: [
        ...examenesDia.map((e: any) => `• Examen: ${e.materia}${e.hora ? ` a las ${e.hora}hs` : ""}${e.aula ? ` (${e.aula})` : ""}`),
        ...trabajosDia.map((t: any) => `• Trabajo: ${t.title}`),
      ],
    });
  }

  // --- Salud: medicamentos que se agotan para ese día ---
  const medsBajoStock: string[] = [];
  for (const medDoc of medsSnap.docs) {
    const med = medDoc.data() as any;
    if (med.estado !== "Consumiendo") continue;
    const dispEntries = dispSnap.docs.map((d: any) => d.data() as any).filter((d: any) => d.medicamentoId === medDoc.id);
    if (dispEntries.length === 0) continue;
    const latest = dispEntries.sort((a: any, b: any) => String(b.fechaRegistro).localeCompare(String(a.fechaRegistro)))[0];
    ...
    const targetDateObj = new Date(targetDateStr + "T00:00:00-03:00");
    const diffDays = Math.max(0, Math.round((targetDateObj.getTime() - regDate.getTime()) / (1000 * 60 * 60 * 24)));
  ...
  if (medsBajoStock.length > 0) {
    sections.push({
      title: "💊 Salud",
      wa: medsBajoStock.map((d) => `• *${d}* se termina ese día`),
      push: medsBajoStock.map((d) => `• ${d} se termina ese día`),
    });
  }

  // --- Comidas: qué toca comer ese día ---
  const comidaDia = orgSnap.docs.map((d: any) => d.data() as any).filter((o: any) => o.fecha === targetDateStr);
  if (comidaDia.length > 0) {
    const platoNames = comidaDia.map((o: any) => {
      const plato = platosSnap.docs.find((p: any) => p.id === o.platoId)?.data() as any;
      return plato?.nombrePlato || "Comida planificada";
    });
    sections.push({
      title: "🍽️ Comidas",
      wa: platoNames.map((n: string) => `• *${n}*`),
      push: platoNames.map((n: string) => `• ${n}`),
    });
  }
  ...
  for (const email of KNOWN_USER_EMAILS) {
    try {
      ...
      if (argHour === DAILY_SUMMARY_HOUR && profile?.whatsappLastSummaryDate !== argDateStr) {
        const sections = await buildSummarySections(argDateStr, now, {
          turnosSnap, paymentsSnap, examenesSnap, tasksSnap, medsSnap, dispSnap, orgSnap, platosSnap,
        });
      ...
      // --- 22 HS: adelanto de lo que tenés para MAÑANA ---
      if (argHour === NIGHT_BEFORE_SUMMARY_HOUR && profile?.whatsappLastNightSummaryDate !== argDateStr) {
        const tomorrow = new Date(now.getTime() + 24 * 60 * 60 * 1000);
        const tomorrowDateStr = getArgentinaDateStr(tomorrow);

        const tomorrowSections = await buildSummarySections(tomorrowDateStr, now, {
          turnosSnap, paymentsSnap, examenesSnap, tasksSnap, medsSnap, dispSnap, orgSnap, platosSnap,
        });

        if (tomorrowSections.length > 0) {
          const waMessage = `🌙 *Esto es lo que tenés para mañana:*\n\n${tomorrowSections.map((s) => `${s.title}\n${s.wa.join("\n")}`).join("\n\n")}`;
          const pushMessage = tomorrowSections.map((s) => `${s.title}\n${s.push.join("\n")}`).join("\n\n");
          await send("🌙 Adelanto de mañana", waMessage, `Esto es lo que tenés para mañana:\n\n${pushMessage}`);
        }

        if (profileDocSnap) {
          await updateDoc(doc(db, "users", email, "user_profile", profileDocSnap.id), { whatsappLastNightSummaryDate: argDateStr });
        }
      }
