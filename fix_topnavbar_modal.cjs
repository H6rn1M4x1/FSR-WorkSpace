...
const startIdx = content.indexOf('<AnimatePresence>');
const endIdx = content.indexOf('</AnimatePresence>') + '</AnimatePresence>'.length;

if (startIdx !== -1 && endIdx !== -1) {
  const replacement = `{showNotifications &&
                createPortal(
                  <div
  ...
  fs.writeFileSync('src/components/TopNavbar.tsx', newContent);
  console.log("TopNavbar updated.");
} else {
  console.log("Could not find TopNavbar target.");
}
