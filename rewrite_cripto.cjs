...
export default function CotizacionesCriptoTable({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newItem: CotizacionCripto = {
      id: editingItem ? editingItem.id : "c_" + Date.now().toString(),
      name: formName,
      price: formPrice,
              ...
              >
                ...
                <div className="p-4 sm:p-5 border-t border-slate-100 dark:border-zinc-800/40 bg-slate-50/50 dark:bg-[#0a0a0a] flex items-center justify-end gap-3 shrink-0">
                  <button
                    type="button"
                    onClick={() => setShowAddModal(false)}
                    className="px-4 py-2.5 rounded-xl text-xs font-bold text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-zinc-800 transition-all cursor-pointer"
                  >
                    Cancelar
                  </button>
                  <button
                    type="submit"
                    form="cripto-form"
                    className="px-5 py-2.5 rounded-xl bg-primary hover:bg-primary-hover text-white text-xs font-bold transition-all shadow-sm cursor-pointer"
                  >
