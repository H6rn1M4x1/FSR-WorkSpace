    ...
    try {
      ...
      if (token) {
        ...
        if (result.success && result.webViewLink) {
          setPayments((prev) =>
            prev.map((p) =>
              p.id === paymentId ? { ...p, [field]: result.webViewLink } : p,
            ),
          );
          alert(
            `¡Archivo "${file.name}" subido y vinculado a tu Google Drive con éxito!`,
          );
        } else {
          alert(
        ...
        setPayments((prev) =>
          prev.map((p) =>
            p.id === paymentId ? { ...p, [field]: base64Data } : p,
          ),
        );
        alert(
          `¡Archivo "${file.name}" cargado localmente! (Inicia sesión con Google para guardarlo en tu Drive).`,
        );
      }
    } catch (err: any) {
                          ...
                          setPayments((prev) =>
                            prev.map((item) =>
                              item.id === p.id
                                ? {
                                    ...item,
                                    pagoRecurrente: !item.pagoRecurrente,
                                  }
                                : item,
                            ),
                          );
