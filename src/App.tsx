...
import type { UserProfileData } from "./components/UserSettingsModal";
...
export default function App() {
  ...
  // Self-heal userProfile.email: it should always match the real logged-in session, never
  // a possibly-stale value from a previously-saved profile document. This also fixes the
  // stored Firestore document permanently, since correcting it here flows into the existing
  // sync-push effect below.
  useEffect(() => {
    if (!user?.email || !userProfile) return;
    if (userProfile.email === user.email) return;
    setUserProfile((prev) => (prev ? { ...prev, email: user.email } : prev));
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      ...
      if (!user?.email) return; // don't refetch under a guessed identity before auth resolves
      refetchCategory(user.email, "user_profile").catch(() => {});
    };
  ...
  // --- Item-level sharing v2: reference-based, both sides edit the same original record ---
  const [itemsSharedWithMe, setItemsSharedWithMe] = useState<any[]>([]);
  const [sharedInData, setSharedInData] = useState<Record<string, any[]>>({});
  const [itemsIShared, setItemsIShared] = useState<any[]>([]);

  useEffect(() => {
    if (!user?.email) {
      setItemsSharedWithMe([]);
      setItemsIShared([]);
      return;
    }
  ...
  // Hydrate the actual data for everything shared with me, straight from its owner's records.
  useEffect(() => {
    if (itemsSharedWithMe.length === 0) {
      setSharedInData({});
      return;
    }
  ...
  // Merge my own records with the ones shared with me, so every screen (Inicio and each
  // section) shows both. Shared-in items carry a __sharedByEmail marker, which the UI uses
  // to draw the "shared" badge and to know whose collection to write edits back to.
  const mergeShared = <T,>(own: T[], category: string): T[] => {
    const incoming = sharedInData[category] || [];
    if (incoming.length === 0) return own;
    const ownIds = new Set((own as any[]).map((i) => i?.id));
    return [...own, ...incoming.filter((i: any) => !ownIds.has(i?.id))] as T[];
  };

  const turnosCompromisosWithShared = mergeShared(turnosCompromisos, "turnos_compromisos");
  const appointmentsWithShared = mergeShared(appointments, "appointments");
  const detailedPaymentsWithShared = mergeShared(detailedPayments, "detailed_payments");
  const invoicesWithShared = mergeShared(invoices, "invoices");
  const organizacionSemanalWithShared = mergeShared(organizacionSemanal, "organizacion_semanal");
  const disponibilidadMedicamentosWithShared = mergeShared(disponibilidadMedicamentos, "disponibilidad_medicamentos");
  // Newly shareable categories
  const tasksWithShared = mergeShared(tasks, "tasks");
  const examenesWithShared = mergeShared(examenes, "examenes");
  const platosWithShared = mergeShared(platos, "platos");
  const shoppingListWithShared = mergeShared(shoppingList, "shopping");
  const pantryWithShared = mergeShared(pantry, "pantry");
  const gastosVariosWithShared = mergeShared(gastosVarios, "gastos_varios");
  const inversionesWithShared = mergeShared(inversiones, "inversiones");
  const budgetsWithShared = mergeShared(budgets, "budgets");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        // Never let a cached email (possibly from a different account that used this same
        // device before) override the currently logged-in user's real email.
        delete parsed.email;
  ...
  // Safety-net background refresh: independent of the "another device changed something"
  // notification, this periodically re-checks every category directly against the server.
  // This guarantees changes made on another device eventually show up here on their own
  // (within ~60s) even if the lighter notification mechanism has an issue.
  useEffect(() => {
    const currentUserId = user?.email || userProfile?.email || "hernanmaximiliano10@gmail.com";
    const categoriesToPoll: Array<{ key: string; targetUserId: string }> = [
      { key: "subjects", targetUserId: currentUserId },
      { key: "tasks", targetUserId: currentUserId },
      { key: "notes", targetUserId: currentUserId },
      { key: "materias_info", targetUserId: currentUserId },
      { key: "pantry", targetUserId: effectiveComidasUserId },
      { key: "meals", targetUserId: effectiveComidasUserId },
      { key: "shopping", targetUserId: effectiveComidasUserId },
      { key: "invoices", targetUserId: currentUserId },
      { key: "payments", targetUserId: currentUserId },
      { key: "detailed_payments", targetUserId: currentUserId },
      { key: "gastos_varios", targetUserId: currentUserId },
      { key: "inversiones", targetUserId: currentUserId },
      { key: "cotizaciones_acciones", targetUserId: currentUserId },
      { key: "cotizaciones_cripto", targetUserId: currentUserId },
      { key: "budgets", targetUserId: currentUserId },
      { key: "appointments", targetUserId: currentUserId },
      { key: "routines", targetUserId: currentUserId },
      { key: "medications", targetUserId: effectiveControlClinicoUserId },
      { key: "blood_pressure", targetUserId: effectiveControlClinicoUserId },
      { key: "doctors", targetUserId: effectiveControlClinicoUserId },
      { key: "mercaderia", targetUserId: effectiveComidasUserId },
      { key: "alimentos", targetUserId: effectiveComidasUserId },
      ...
      { key: "rutinas_gimnasio", targetUserId: currentUserId },
      { key: "registros_entrenamiento", targetUserId: currentUserId },
      { key: "medical_records", targetUserId: effectiveControlClinicoUserId },
      { key: "alimentacion_logs", targetUserId: currentUserId },
      { key: "horarios", targetUserId: currentUserId },
      { key: "examenes", targetUserId: currentUserId },
    ];

    const runPollNow = () => {
      categoriesToPoll.forEach((col) => {
        refetchCategory(col.targetUserId, col.key, true).catch(() => {});
      });
    };

    const intervalId = setInterval(() => {
      // Only poll while the tab is actually visible — no point burning reads in the background.
      if (document.visibilityState !== "visible") return;
      runPollNow();
    }, 60000); // every 60 seconds

    // Mobile browsers/OS often pause JS (and any pending Firestore notifications) while the
    // app is backgrounded or the screen is off. Force an immediate refresh the moment the
    // app becomes visible again (unlocking the phone, switching back to the tab), instead of
    // waiting for the next 60s tick — this is what fixes the "stale until I switch sections"
    // symptom without needing any user action at all.
    const handleVisible = () => {
      if (document.visibilityState === "visible") runPollNow();
    };
      ...
      <div className="flex-1 flex flex-col min-w-0 z-10 pt-3 sm:pt-5">
        ...
        <main className="flex-1 overflow-y-auto">
          {driveLoading || !isInitialLoadDone ? (
            ...
            <Suspense fallback={<PageSkeleton tab={currentTab} darkMode={darkMode} />}>
            <>
              {currentTab === "home" && (
                <HomeView
                  darkMode={darkMode}
                  ...
                  itemsIShared={itemsIShared}
                  invoices={invoicesWithShared}
                  setInvoices={setInvoices}
                  detailedPayments={detailedPaymentsWithShared}
                  ...
                  token={token}
                  setToken={setToken}
                  disponibilidadMedicamentos={disponibilidadMedicamentosWithShared}
                  medicamentosDetallados={medicamentosDetallados}
                  notes={notes}
                  horarios={horarios}
                  examenes={examenes}
              ...
              {currentTab === "meals" && (
                <MealsView
                  ...
                  pantry={pantryWithShared}
                  setPantry={setPantry}
                  meals={meals}
                  setMeals={setMeals}
                  shoppingList={shoppingListWithShared}
                  setShoppingList={setShoppingList}
                  mercaderia={mercaderia}
                  setMercaderia={setMercaderia}
                  alimentos={alimentos}
                  setAlimentos={setAlimentos}
                  platos={platosWithShared}
                  setPlatos={setPlatos}
                  organizacionSemanal={organizacionSemanalWithShared}
                  setOrganizacionSemanal={setOrganizacionSemanal}
                  activeSubTab={activeSubTab}
              ...
              {currentTab === "finances" && (
                <FinanceView
                  ...
                  detailedPayments={detailedPaymentsWithShared}
                  setDetailedPayments={setDetailedPayments}
                  gastosVarios={gastosVariosWithShared}
                  setGastosVarios={setGastosVarios}
                  inversiones={inversionesWithShared}
