export class WorkspaceService {
  ...
  static async syncCalendarEvents(
    ...
    accessToken: string
  ): Promise<{ success: boolean; syncedCount?: number; errors?: string[]; error?: string }> {
    if (!accessToken) {
      return { success: false, error: "No se proporcionó token de Google Workspace." };
    }
    try {
      const res = await fetch("/api/workspace/sync-calendar", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${accessToken}`,
      ...
      const data = await res.json();
      return { success: true, syncedCount: data.syncedCount, errors: data.errors };
    } catch (err: any) {
