export class WorkspaceService {
  ...
  // Updates an existing Google Calendar event in place (keeps the same event, just
  // refreshes its title/description/date/etc). Returns notFound: true if the event was
  // deleted directly in Google Calendar, so the caller can decide to re-create it.
  static async updateCalendarEvent(
    googleEventId: string,
    eventData: { title?: string; description?: string; location?: string; date: string; time?: string },
    accessToken: string
    ...
    try {
      const res = await fetch("/.netlify/functions/update-calendar-event", {
        ...
        body: JSON.stringify({ googleEventId, ...eventData }),
      });
      if (res.status === 404) {
        return { success: false, notFound: true };
      }
      if (!res.ok) {
        let errMessage = "Error al actualizar el evento de Google Calendar.";
        try {
          const err = await res.json();
          errMessage = err.error || errMessage;
        } catch {}
      ...
      return { success: false, error: err?.message || "Error de conexión al actualizar el evento." };
    }
  }
}
