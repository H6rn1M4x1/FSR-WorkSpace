...
export default function MealsView({
  ...
  // NOTE: pantry, meals, shopping, mercaderia, alimentos, platos and organizacion_semanal
  // used to have their own duplicate real-time Firestore subscription here — it also
  // didn't account for shared "comidas" sections. They already arrive as live, correctly
  // targeted props from App.tsx's single central subscription, so this was removed.
                      ...
                      >
                        <td className="py-3.5 px-4 whitespace-nowrap font-semibold text-slate-900 dark:text-white md:sticky md:left-0 z-20 bg-white dark:bg-zinc-950 group-hover:bg-slate-50 dark:group-hover:bg-zinc-900 transition-colors min-w-[240px] w-[240px] max-w-[240px]">
                          <span className="flex items-center gap-1.5 min-w-0">
                            <span className="truncate" title={item.nombrePlato}>{item.nombrePlato}</span>
                            <SharedBadge
                              item={item}
                              isSharedOut={isSharedOut("platos", item.id)}
                              sharedOutWith={sharedOutWith("platos", item.id)}
                            />
                          </span>
                        </td>
