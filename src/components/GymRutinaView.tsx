...
export const GymRutinaView: React.FC<GymRutinaViewProps> = ({
  ...
  // Same as combinedHistory, but respecting the day filter — used for the "Todos/GYM/Deportes"
  // badge counts, so they match what's actually shown in the list below instead of always
  // showing the grand total regardless of the selected day.
  const dateFilteredCombinedHistory = useMemo(() => {
    if (!selectedFilterDate) return combinedHistory;
    return combinedHistory.filter((item) => new Date(item.fecha).toDateString() === selectedFilterDate.toDateString());
  }, [combinedHistory, selectedFilterDate]);
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 p-1 bg-slate-100 dark:bg-zinc-900 rounded-2xl border border-slate-200/60 dark:border-zinc-800 shrink-0">
              ...
              >
                Todos ({dateFilteredCombinedHistory.length})
              </button>
              <button
                type="button"
              ...
              >
                <Dumbbell className="w-3 h-3" /> GYM ({dateFilteredCombinedHistory.filter(i => i.tipo === "gym").length})
              </button>
              <button
                type="button"
              ...
              >
                <Activity className="w-3 h-3" /> Deportes ({dateFilteredCombinedHistory.filter(i => i.tipo === "actividad").length})
              </button>
            </div>
          </div>
