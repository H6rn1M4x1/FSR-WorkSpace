...
export default function HealthView({
  ...
  const handleSyncGoogleFit = async () => {
    ...
    try {
      if (!token) {
        ...
        showToast("Renovando el permiso de Google, probá de nuevo en un momento.", "error");
        return;
      }
      const { fetchNewGoogleFitSessions } = await import("../lib/googleFitService");
      const existingIds = new Set(
        (deportesActividades || [])
          .map((a: any) => a.googleFitSessionId)
          .filter(Boolean)
      );
      const nuevas = await fetchNewGoogleFitSessions(token, existingIds);

      if (nuevas.length === 0) {
        showToast("No hay entrenamientos nuevos en Google Fit para importar.", "success");
        return;
      }
      ...
      for (const act of nuevas) {
        await saveItemToFirestore(activeUserId, "deportes_actividades", act);
      }
      setDeportesActividades?.((prev) => [...(prev || []), ...nuevas]);
      showToast(`${nuevas.length} entrenamiento(s) importado(s) de Google Fit.`, "success");
    } catch (err: any) {
      showToast(err?.message || "Error al sincronizar con Google Fit.", "error");
    } finally {
  ...
  // Real-time local subscription for HealthView-only categories with strict unmount cleanup.
  // NOTE: medications, blood_pressure, doctors, medicamentos_detallados,
  // disponibilidad_medicamentos, deportes_actividades, rutinas_gimnasio,
  // registros_entrenamiento, medical_records and alimentacion_logs used to also be
  // subscribed here — they already arrive as live props from App.tsx's central
  // subscription (which also correctly handles shared "control_clinico" sections,
  // unlike this local one), so that duplicate part was removed. metabolic_profile,
  // hydration_logs and medidas_history stay here since they aren't subscribed elsewhere.
  useEffect(() => {
    const activeUserId = (userEmail || auth.currentUser?.email || auth.currentUser?.uid || "hernanmaximiliano10@gmail.com").toLowerCase().trim();
    const unsubs = [
          ...
          <div className={`p-6 rounded-3xl border ${darkMode ? "bg-zinc-900/40 border-white/10 backdrop-blur-md" : "bg-white/40 border-slate-200 backdrop-blur-md"}`}>
            ...
            >
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <h3 className="font-extrabold text-sm">Historial de Sesiones</h3>
                <button
                  type="button"
                  onClick={handleSyncGoogleFit}
                  disabled={isSyncingFit}
                  className="px-3 py-1.5 rounded-full bg-primary hover:bg-primary/90 text-white dark:text-blue-950 font-bold text-[11px] cursor-pointer transition-all shadow-sm flex items-center gap-1.5 disabled:opacity-50"
                >
                  {isSyncingFit ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <RefreshCw className="w-3.5 h-3.5" />}
                  <span>{isSyncingFit ? "Sincronizando..." : "Sincronizar con Google Fit"}</span>
                </button>
              </div>
              {(() => {
