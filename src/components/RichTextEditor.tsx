...
export function RichTextEditor({ value, onChange, placeholder, onShareClick, attachments, onAttachmentsChange, onPreview }: RichTextEditorProps) {
  ...
  // Sync initial value only once or if it's completely cleared externally
  useEffect(() => {
    if (editorRef.current && editorRef.current.innerHTML !== value) {
      if (!value) {
        editorRef.current.innerHTML = '';
      }
      // If we want to support full sync, we need to manage cursor position, 
      // which is complex. For now, we only sync when value is empty (reset).
    }
  }, [value]);
