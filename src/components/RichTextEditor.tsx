      ...
      <div 
        ...
        onKeyDown={(e) => {
          if (e.key === 'Tab') {
            ...
            let checklistLine: HTMLElement | null = null;
            if (selection && selection.rangeCount > 0) {
              let currentNode: Node | null = selection.anchorNode;
              if (currentNode?.nodeType === 3) currentNode = currentNode.parentNode;
              const parentEl = currentNode as HTMLElement | null;
              const closestDiv = parentEl?.closest("div") || null;
              if (closestDiv && closestDiv !== editorRef.current && closestDiv.querySelector('input[type="checkbox"]')) {
                checklistLine = closestDiv;
              }
            }
            if (checklistLine) {
              const INDENT_STEP = 24; // px per indent level
              const currentIndent = parseInt(checklistLine.style.marginLeft || "0", 10) || 0;
              const nextIndent = e.shiftKey
                ? Math.max(0, currentIndent - INDENT_STEP)
                : currentIndent + INDENT_STEP;
              checklistLine.style.marginLeft = `${nextIndent}px`;
              handleInput();
            } else if (e.shiftKey) {
              execCommand("outdent");
            } else {
            ...
            if (selection && selection.rangeCount > 0) {
              ...
              if (parentEl) {
                ...
                if (lineDiv && lineDiv.querySelector('input[type="checkbox"]')) {
                   ...
                   } else if (lineDiv === editorRef.current) {
                     // Safety net: this checkbox wasn't wrapped in its own line div
                     // (e.g. created before this fix) — inserting as a sibling of the
                     // whole editor would place it outside the editable area entirely.
                     // Fall back to inserting right at the cursor instead.
                     const html = `<div><input type="checkbox" style="margin-right: 6px; cursor: pointer;" />&nbsp;</div>`;
                     execCommand("insertHTML", html);
                   } else {
