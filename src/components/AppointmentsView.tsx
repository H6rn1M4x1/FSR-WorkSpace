...
const MapComponent = ({
  ...
  // Only auto-fit/center the map the first time markers are drawn — later marker refreshes
  // (e.g. from the background data sync) must NOT reset whatever zoom/pan the user set.
  ...
  useEffect(() => {
    ...
    Object.entries(locationsWithTurnos).forEach(([lugar, tcs]) => {
      ...
      if (markerCount > 0 && mapInstanceRef.current && markersLayerRef.current && !hasFitBoundsOnceRef.current) {
        try {
      ...
      } else if (mapInstanceRef.current && !hasFitBoundsOnceRef.current) {
        try {
...
export default function AppointmentsView({
  ...
  const handleSelectFromAddressBook = (entry: AddressEntry) => {
    setTcLugar(entry.name);
    setLugarQuery(entry.name);
    setTcLat(entry.lat);
    setTcLon(entry.lon);
  };
    ...
    if (placesToFetch.length > 0) {
      placesToFetch.forEach(async (placeName) => {
        try {
          ...
          const res = await fetch(`/.netlify/functions/geocode-place?q=${encodeURIComponent(query)}`);
          if (res.ok) {
  ...
  const validLocationsCount = useMemo(() => {
    ...
    (turnosCompromisos || []).forEach((tc) => {
      if (String(tc?.id || "").startsWith("match-")) return; // football match stadiums don't belong on this map
  ...
  const handlePlacesSearch = async (query: string) => {
    ...
    searchDebounceRef.current = setTimeout(async () => {
      try {
        let url = `/.netlify/functions/geocode-place?q=${encodeURIComponent(query.trim())}`;
        if (userCoords) {
  ...
  const handleAddTurnoCompromiso = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isWithoutAddress) {
        ...
        try {
          ...
          const res = await fetch(
            `/.netlify/functions/geocode-place?q=${encodeURIComponent(query)}`,
      ...
      if (editingTurnoComp) {
        ...
        // Keep the linked Google Calendar event in sync with the edit.
        (async () => {
          try {
            if (!token) {
              const { silentGoogleReauth } = await import("../lib/supabase");
              await silentGoogleReauth();
              return;
            }
            ...
            const existingGoogleId = (payload as any).googleCalendarEventId;
            if (existingGoogleId) {
              const result = await WorkspaceService.updateCalendarEvent(existingGoogleId, eventData, token);
              if (result.notFound) {
                // Was deleted directly in Google Calendar — recreate it.
                const created = await WorkspaceService.syncCalendarEvents(
                  [{ id: `tc-${payload.id}`, ...eventData }],
                  token,
                );
                const newId = created.eventIds?.[`tc-${payload.id}`];
                if (newId) {
                  const withCalendarId = { ...payload, googleCalendarEventId: newId };
              ...
              // Never had a linked event (e.g. created before this feature) — create one now.
              const created = await WorkspaceService.syncCalendarEvents(
                [{ id: `tc-${payload.id}`, ...eventData }],
                token,
              );
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 py-2.5 px-4 bg-slate-50 dark:bg-black/40 border border-slate-150 dark:border-zinc-800/50 rounded-2xl mb-4 mt-3">
                  ...
                  <PillFilterBar
                    ...
                    activeValue={agendaStatusFilter}
                    onChange={setAgendaStatusFilter}
                ...
                <AnimatePresence mode="wait">
                  ...
                  {sortedTurnosCompromisos.filter((tc) => {
                    ...
                    const matchesStatus =
                      ...
                      (agendaStatusFilter === "Pendiente"
                        ? estado === "Pendiente" || estado === "Initinere Diario"
                        : estado === agendaStatusFilter);
                                  ...
                                  return (
                                    <div className="flex items-center gap-2 mt-1.5 min-w-0">
                                      ...
                                      >
                                        <input
                                          ...
                                          onChange={async (e) => {
                                            ...
                                            const updatedItem = { ...tc, estatus: newChecked, updatedAt: Date.now() };
                                            setTurnosCompromisos((prev) => {
                                              const updated = prev.map((item) =>
                                                item.id === tc.id ? updatedItem : item,
                                              );
                                              StorageService.setTurnosCompromisos(updated);
                                              return updated;
                                            });
                                            try {
                                              await saveItemToFirestore(activeUserId, "turnos_compromisos", updatedItem);
                                              showToast(
                                                newChecked ? "Marcado como listo y guardado." : "Marcado como pendiente y guardado.",
                                                "success",
                                              );
                                            } catch (err: any) {
                                              setTurnosCompromisos((prev) => {
                                                const reverted = prev.map((item) => (item.id === tc.id ? tc : item));
                                      ...
                                      >
                                        ...
                                        {(tc as any).__sharedByEmail && (
                                          <span
                                            title={`Compartido por ${(tc as any).__sharedByName || (tc as any).__sharedByEmail}`}
                                            className="shrink-0 self-center inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-primary/15 text-primary text-[9px] font-extrabold max-w-[120px]"
                                          >
                                            <Share2 className="w-2.5 h-2.5 shrink-0" />
                                            <span className="truncate">
                                              {((tc as any).__sharedByName || (tc as any).__sharedByEmail || "").split("@")[0]}
                                            </span>
                                          </span>
                                        )}
                                      </h4>
                                ...
                                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] text-zinc-400 mt-1.5 font-medium">
                                  ...
                                  {tc.lugar &&
                                    ...
                                    <span className="flex items-center gap-1">
                                      ...
                                      <a
                                        href={tc.lat != null && tc.lon != null ? `https://www.openstreetmap.org/?mlat=${tc.lat}&mlon=${tc.lon}#map=17/${tc.lat}/${tc.lon}` : `https://www.openstreetmap.org/search?query=${encodeURIComponent(tc.lugar)}`}
                                        target="_blank"
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          >
                            ...
                            <span>Abrir Mapa</span>
                                    ...
                                    {/* Información Personalizada — no aplica a turnos de partido (guardan JSON, no texto libre) */}
                  {!editingTurnoComp?.id?.startsWith("match-") && (
                  <div>
                      ...
                      {item.lugar &&
                        ...
                        <div className="flex items-start gap-2 text-zinc-500 dark:text-zinc-400 font-bold">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <a
                              href={item.lat != null && item.lon != null ? `https://www.openstreetmap.org/?mlat=${item.lat}&mlon=${item.lon}#map=17/${item.lat}/${item.lon}` : `https://www.openstreetmap.org/search?query=${encodeURIComponent(item.lugar)}`}
                              target="_blank"
                              rel="noopener noreferrer"
