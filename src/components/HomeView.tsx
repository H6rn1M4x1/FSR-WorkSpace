...
export default function HomeView({
  ...
  const isItemSharedOut = (category: string, itemId: string): boolean =>
    (itemsIShared || []).some((s: any) => s.category === category && s.itemId === itemId);

  const sharedOutWithNames = (category: string, itemId: string): string =>
    (itemsIShared || [])
      .filter((s: any) => s.category === category && s.itemId === itemId)
      .map((s: any) => s.sharedWithEmail)
      .join(", ");
  ...
  const handleShareSpecificItem = async (
    ...
    if (email === user.email.toLowerCase()) {
      showToast("No podés compartir algo con vos mismo.", "error");
      return;
    }

    // Map the UI's item type to the Firestore category the record actually lives in.
    ...
    const category = categoryByType[itemType];
  ...
  const handleToggleShareForUser = async (
    ...
    try {
      ...
      showToast(
        isShared ? `Dejaste de compartir con ${share.toEmail}.` : `Compartido con ${share.toEmail}.`,
        "success",
      );
    } catch (err) {
  ...
  const handleToggleTurnoStatus = async (id: string) => {
    const original = turnosCompromisos.find((item) => item.id === id);
    if (!original) return;
    const updatedItem = { ...original, estatus: !original.estatus, updatedAt: Date.now() };
    ...
    try {
      const activeUserId = user?.email || userProfile?.email || "hernanmaximiliano10@gmail.com";
      await saveItemToFirestore(activeUserId, "turnos_compromisos", updatedItem);
      showToast(updatedItem.estatus ? "Marcado como listo y guardado." : "Marcado como pendiente y guardado.", "success");
    } catch (err: any) {
      setTurnosCompromisos((prev) => {
        const reverted = prev.map((item) => (item.id === id ? original : item));
  ...
  const handleToggleDetailedPaymentStatus = async (id: string) => {
    const original = detailedPayments.find((item) => item.id === id);
    if (!original) return;
    const updatedItem = { ...original, pago: !original.pago, updatedAt: Date.now() };
    setDetailedPayments((prev) => {
      const updated = prev.map((item) => (item.id === id ? updatedItem : item));
    ...
    try {
      const activeUserId = user?.email || userProfile?.email || "hernanmaximiliano10@gmail.com";
      await saveItemToFirestore(activeUserId, "detailed_payments", updatedItem);
      showToast(updatedItem.pago ? "Marcado como pagado y guardado." : "Marcado como pendiente y guardado.", "success");
    } catch (err: any) {
      setDetailedPayments((prev) => {
        const reverted = prev.map((item) => (item.id === id ? original : item));
        StorageService.setDetailedPayments(reverted);
        return reverted;
      });
      showToast(err?.message || "No se pudo guardar el cambio.", "error");
    }
  ...
  const handleToggleInvoiceStatus = async (id: string) => {
    const original = invoices.find((item) => item.id === id);
    if (!original) return;
    const updatedItem = { ...original, paid: !original.paid, updatedAt: Date.now() };
    setInvoices((prev) => {
    ...
    try {
      const activeUserId = user?.email || userProfile?.email || "hernanmaximiliano10@gmail.com";
      await saveItemToFirestore(activeUserId, "invoices", updatedItem);
      showToast(updatedItem.paid ? "Marcado como pagado y guardado." : "Marcado como pendiente y guardado.", "success");
    } catch (err: any) {
      setInvoices((prev) => {
        const reverted = prev.map((item) => (item.id === id ? original : item));
        StorageService.setInvoices(reverted);
        return reverted;
      });
      showToast(err?.message || "No se pudo guardar el cambio.", "error");
    }
                              ...
                              {matchLogos && (matchLogos.homeLogo || matchLogos.awayLogo) ? (
                                ...
                                <div className="flex items-center gap-2 mt-1.5 min-w-0">
                                  ...
                                  >
                                    ...
                                    <SharedBadge
                                      item={tc}
                                      isSharedOut={isItemSharedOut("turnos_compromisos", tc.id)}
                                      sharedOutWith={sharedOutWithNames("turnos_compromisos", tc.id)}
                                    />
                                  </h4>
                                </div>
                              )}
          ...
          <>
            ...
            >
              ...
              {(() => {
                ...
                const cat = categoryByType[contextMenu.itemType];
                const itemId =
                  contextMenu.itemType === "medication"
                    ? contextMenu.data?.disp?.id
                    : contextMenu.data?.id;
                const currentShares = (itemsIShared || []).filter(
                  (s: any) => s.category === cat && s.itemId === itemId,
                );
                if (currentShares.length === 0) return null;

                return (
                  <div className="mt-1 pt-1 border-t border-zinc-200 dark:border-zinc-800">
                    <p className="px-3 py-1 text-[9px] font-extrabold uppercase tracking-wider text-zinc-400">
                      Compartido con
                    </p>
                    {currentShares.map((s: any) => (
                      <button
                        key={s.id}
                        onClick={async () => {
                          try {
                        ...
                        className="w-full flex items-center justify-between gap-2 px-3 py-2 text-xs font-bold hover:bg-red-500/10 hover:text-red-500 rounded-full transition-all text-left cursor-pointer"
                      >
                        <span className="truncate">{s.sharedWithEmail}</span>
                        <span className="shrink-0 text-[9px] font-extrabold uppercase">Quitar</span>
                      </button>
                    ))}
                  </div>
                );
