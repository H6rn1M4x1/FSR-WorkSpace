...
import { subscribeToCategory } from "../lib/firestoreSyncService";
...
export default function HomeView({
  ...
  // Real-time local subscription for HomeView categories with strict unmount cleanup
  useEffect(() => {
    ...
    const unsubs = [
      ...
      subscribeToCategory(userId, "detailed_payments", (items) => setDetailedPayments?.(items)),
      subscribeToCategory(userId, "appointments", (items) => setAppointments?.(items)),
      subscribeToCategory(userId, "turnos_compromisos", (items) => setTurnosCompromisos?.(items)),
  ...
  const handleScheduleMatches = async () => {
    ...
    setTimeout(() => setScheduleSuccessMsg(null), 6000);
  };
  ...
  const handleGoogleCalendarSync = async (scope: "day" | "month") => {
    if (!token) {
      try {
        ...
        if (res && res.accessToken && setToken) {
        ...
        setSyncStatus({
          loading: false,
          error: "No se pudo vincular la cuenta: " + err.message,
        });
        return;
      }
    }
  ...
  return (
    <div className="space-y-6 animate-fade-in px-3 sm:px-6 pt-1 sm:pt-1.5 pb-6" id="home-view-container">
      {scheduleSuccessMsg && (
        <div className="p-4 rounded-2xl bg-primary/10 border border-primary/30 text-primary text-xs font-bold flex items-center justify-between gap-3 animate-fade-in shadow-sm">
          ...
          <button
            ...
            className="p-1 text-primary hover:bg-primary/20 rounded-lg cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
