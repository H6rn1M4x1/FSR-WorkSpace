...
export default function PaymentsTable({
  ...
  // Uploads a file to Google Drive if logged in (returns the Drive link), or falls back
  // to a base64 data URL stored directly in the record. Used by both the table's inline
  // "Subir" buttons and the Add/Edit Pago form.
  const uploadFileGetUrl = async (file: File): Promise<{ url: string; viaGoogleDrive: boolean }> => {
    ...
    if (token) {
      ...
      if (result.success && result.webViewLink) {
        return { url: result.webViewLink, viaGoogleDrive: true };
      }
      throw new Error(result.error || "Error desconocido al subir a Google Drive");
    }
    return { url: base64Data, viaGoogleDrive: false };
  };

  // Opens a stored file. Plain http(s) links (Google Drive) open directly, but base64
  // data: URLs often show a blank tab if opened via a normal link — browsers restrict
  // navigating to long data: URIs. Converting to a Blob object URL first fixes that.
  ...
  const handleFileUpload = async (
    ...
    try {
      const userId = userEmail || "hernanmaximiliano10@gmail.com";
      const { url, viaGoogleDrive } = await uploadFileGetUrl(file);

      const updatedPayment = payments.find((p) => p.id === paymentId);
      if (updatedPayment) {
        const toSave = { ...updatedPayment, [field]: url };
        setPayments((prev) =>
      ...
      showToast(
        viaGoogleDrive
          ? `Archivo "${file.name}" subido y guardado con éxito.`
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let facturaEmitida = existingPayment?.facturaEmitida;
      let comprobantePago = existingPayment?.comprobantePago;

      if (facturaFile) {
        const { url } = await uploadFileGetUrl(facturaFile);
        facturaEmitida = url;
      }
      if (comprobanteFile) {
        const { url } = await uploadFileGetUrl(comprobanteFile);
        comprobantePago = url;
      }
                    ...
                    <td className="px-6 py-4 text-center min-w-[120px]">
                      {uploadingState[`${p.id}-facturaEmitida`] ? (
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            type="button"
                            onClick={() => openStoredFile(p.facturaEmitida!, `factura-${p.descripcion}`)}
                            className="flex items-center gap-1 px-2 py-1 bg-primary/10 dark:bg-primary/10 text-primary dark:text-primary rounded-lg text-[10px] font-extrabold hover:underline transition-all cursor-pointer"
                          >
                    ...
                    <td className="px-6 py-4 text-center min-w-[120px]">
                      {uploadingState[`${p.id}-comprobantePago`] ? (
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            type="button"
                            onClick={() => openStoredFile(p.comprobantePago!, `comprobante-${p.descripcion}`)}
                            className="flex items-center gap-1 px-2 py-1 bg-primary/10 dark:bg-primary/10 text-primary dark:text-primary rounded-lg text-[10px] font-extrabold hover:underline transition-all cursor-pointer"
                          >
                            <FileDown className="w-3 h-3" />
                            <span>Ver</span>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <label className="flex items-center justify-center gap-2 w-full px-3 py-2 rounded-full bg-slate-50 dark:bg-zinc-950/50 border border-slate-200 dark:border-zinc-800 text-slate-900 dark:text-white outline-none transition-all text-sm cursor-pointer hover:bg-slate-100 dark:hover:bg-zinc-900/80">
                        ...
                        <span className="truncate">
                          {facturaFile?.name || (editingPaymentId && payments.find((p) => p.id === editingPaymentId)?.facturaEmitida ? "Reemplazar archivo" : "Subir")}
                        </span>
                        <input
                          type="file"
                          className="hidden"
                    ...
                    <div>
                      ...
                      <label className="flex items-center justify-center gap-2 w-full px-3 py-2 rounded-full bg-slate-50 dark:bg-zinc-950/50 border border-slate-200 dark:border-zinc-800 text-slate-900 dark:text-white outline-none transition-all text-sm cursor-pointer hover:bg-slate-100 dark:hover:bg-zinc-900/80">
                        <Upload className="w-4 h-4 text-slate-400 dark:text-zinc-500" />
                        <span className="truncate">
                          {comprobanteFile?.name || (editingPaymentId && payments.find((p) => p.id === editingPaymentId)?.comprobantePago ? "Reemplazar archivo" : "Subir")}
                        </span>
                        <input
                          type="file"
                          className="hidden"
