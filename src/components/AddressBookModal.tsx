...
export function loadSavedAddressBook(): AddressEntry[] {
  try {
    ...
    if (raw === null) {
      // Never saved anything before on this device — seed with defaults.
      localStorage.setItem(STORAGE_KEY, JSON.stringify(DEFAULT_ADDRESSES));
      return DEFAULT_ADDRESSES;
    }
    const parsed = JSON.parse(raw);
    // A validly-empty array means the user deleted everything on purpose — keep it empty.
    // Only fall back to defaults if the saved value is corrupted/not an array at all.
...
export const AddressBookModal: React.FC<AddressBookModalProps> = ({
  ...
  useEffect(() => {
    if (!isOpen) return;
    // Show the local copy instantly (no delay), then sync with Firestore in the background.
    ...
    const seededFlagKey = `address_book_seeded_v1_${activeUserId}`;
    const unsub = subscribeToCategory(activeUserId, "address_book", (items: AddressEntry[]) => {
      if (items && items.length > 0) {
        setEntries(items);
        try {
          localStorage.setItem(STORAGE_KEY, JSON.stringify(items));
          localStorage.setItem(seededFlagKey, "true"); // there's real data now, never auto-seed again
        } catch (_) {}
      } else if (!localStorage.getItem(seededFlagKey)) {
        // Genuinely the first time ever for this account (not "user deleted everything") —
        // seed Firestore with the defaults so every device starts from the same list.
        try {
          localStorage.setItem(seededFlagKey, "true");
        } catch (_) {}
        DEFAULT_ADDRESSES.forEach((entry) => {
          saveItemToFirestore(activeUserId, "address_book", entry).catch(() => {});
        });
      }
      // If items is empty AND the flag is already set, the user intentionally deleted
      // everything — leave it empty, don't re-seed.
    });

    // Force a real server round-trip right now too, so reopening the modal right after a
    // delete never shows a stale cached copy from before the delete.
    refetchCategory(activeUserId, "address_book", true).catch(() => {});
  ...
  const handleToggleFavorite = (id: string, e: React.MouseEvent) => {
    ...
    const changedEntry = updated.find((item) => item.id === id);
    if (changedEntry) {
      const activeUserId = (auth.currentUser?.email || auth.currentUser?.uid || "hernanmaximiliano10@gmail.com").toLowerCase().trim();
      saveItemToFirestore(activeUserId, "address_book", changedEntry)
        .then(() => showToast(changedEntry.isFavorite ? "Marcado como favorito" : "Quitado de favoritos", "success"))
        .catch(() => showToast("No se pudo guardar el cambio.", "error"));
    }
  };
  ...
  const handleSaveEntry = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let changedEntry: AddressEntry;

      if (editingId) {
        changedEntry = {
          ...(entries.find((item) => item.id === editingId) as AddressEntry),
          name: newName.trim(),
        ...
        updated = entries.map((item) => (item.id === editingId ? changedEntry : item));
      } else {
        changedEntry = {
        ...
        updated = [changedEntry, ...entries];
      }

      setEntries(updated);
      saveAddressBook(updated);
      const activeUserId = (auth.currentUser?.email || auth.currentUser?.uid || "hernanmaximiliano10@gmail.com").toLowerCase().trim();
      await saveItemToFirestore(activeUserId, "address_book", changedEntry);
      showToast(editingId ? "Dirección actualizada con éxito" : "Dirección guardada con éxito", "success");
      setShowAddForm(false);
      setEditingId(null);
      setNewName("");
      setNewAddress("");
      ...
      <ConfirmationModal
        ...
        onConfirm={async () => {
          if (confirmDeleteId) {
            ...
            saveAddressBook(updated);
            const activeUserId = (auth.currentUser?.email || auth.currentUser?.uid || "hernanmaximiliano10@gmail.com").toLowerCase().trim();
            deleteItemFromFirestore(activeUserId, "address_book", confirmDeleteId)
              .then(() => showToast("Dirección eliminada con éxito", "success"))
              .catch(() => showToast("No se pudo eliminar la dirección.", "error"));
