...
export const AddressBookModal: React.FC<AddressBookModalProps> = ({
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToCategory(activeUserId, "address_book", (items: AddressEntry[]) => {
      if (items && items.length > 0) {
        ...
        // First time ever for this account — seed Firestore with the defaults so every
        // device starts from the same list.
  ...
  const handleToggleFavorite = (id: string, e: React.MouseEvent) => {
    ...
    if (changedEntry) {
      const activeUserId = (auth.currentUser?.email || auth.currentUser?.uid || "hernanmaximiliano10@gmail.com").toLowerCase().trim();
      saveItemToFirestore(activeUserId, "address_book", changedEntry).catch(() => {});
    }
  };
      ...
      <ConfirmationModal
        ...
        onConfirm={async () => {
          if (confirmDeleteId) {
            ...
            saveAddressBook(updated);
            const activeUserId = (auth.currentUser?.email || auth.currentUser?.uid || "hernanmaximiliano10@gmail.com").toLowerCase().trim();
            deleteItemFromFirestore(activeUserId, "address_book", confirmDeleteId).catch(() => {});
